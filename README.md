...
| `eka get <target>` | Machine retrieval interface: canonical CKO JSON (stable schema `eka-cko-v1`) emitted on stdout — identity lookup (`<ns>/<type>:<id>[:<v>]`, namespace required) or Engineering Domain query (`discovery` … `operations` → domain collection sorted by canonical form); generated directly from Canonical Knowledge Objects via the Runtime API, never rendered, never parsed from Markdown; stdout carries only the JSON; exit 0 / 1 (no workspace, unregistered repository) / 2 (usage, unknown identity, internal); read-only, never creates a workspace; requires a registered + synced repository (`eka sync` first). |
...
cmd/               CLI command layer (Cobra): root, init, validate, export, import, view, watch, get, sync, project, status
...
machine/           Application layer: the machine interface serializer — CKO → canonical JSON, schema eka-cko-v1 (public package, ADR-015)
...
The application packages (`bootstrap/`, `exchange/`, `conformance/`, `view/`, `machine/`, `runtime/`) are public and reusable independently of the CLI — by SDKs, MCP integrations, or other tools; `compile/`, `workspace/`, `store/` and `sync/` are kernel-internal behind the runtime services since ADR-014 (consumers talk to the Runtime Kernel, never to storage internals).
