# EKA Naming and Terminology Specification v1.1

| Field | Value |
|---|---|
| **Version** | 1.1 |
| **Revision** | v1.1 — additive terminology revision (Engineering Domain, Knowledge Stratum, Representation Alias; Conformance Rules R0–R12); v1.0 content unchanged |
| **Status** | Ratified |
...
**Reading this document:** capitalized terms refer to canonical definitions (EKA Core Specification v1.1 Section 3 + glossary) or to terms defined in this document (Section 2.3). "must" = binding requirement. "should" = recommendation within the bounds of the contract. "may" = option within the bounds of the contract. The EKA Core Specification remains the authoritative conceptual model; this document governs naming and terminology only.
...
- the content of the Conformance Rules themselves (R0–R12); this document names them, it does not redefine them;
...
| **Conformance Rules** | the canonical rule set R0–R12: R1–R9 are the nine numbered rules of the EKA Exchange Specification §14.2; R0 is the structural rule defined by the Reference Validator; R10–R12 are the domain-aware rules defined by EKA Core Specification v1.1 §8.1. |
...
| **Engineering Domain** | the home classification of an Artifact: the stratum-aligned category of engineering knowledge to which the Artifact belongs. Classification property (P15 — reclassification never touches Identity); derived from the Artifact's Knowledge Dimension and token family, or declared by an extension; never Identity, never part of the State Vector (EKA Core Specification v1.1 §8.1). |
| **Knowledge Stratum** | the authority level of an Engineering Domain: a fixed position in the strict linear order Discovery → Architecture → Planning → Execution → Operations (stratum 1 highest → 5). Always derived from the Engineering Domain — never declared by an Artifact, never part of the State Vector (EKA Core Specification v1.1 §8.1). |
| **Representation Alias** | a methodology term mapped onto a canonical token + Engineering Domain (e.g., PRD → req-, Sprint/Iteration → ctr-); never a frontmatter value, never an Artifact type in its own right (EKA Core Specification v1.1 §8.1). |
...
| Versioned form | "EKA v1.1" (version applies to the standard corpus) |
...
- Title: **EKA \<Family\> Specification v\<major\>.\<minor\>** — e.g., "EKA Core Specification v1.1", "EKA Exchange Specification v1.0".
...
- The Core Family omits the token: `eka-specification-v1.1.md`.
- The meta-specification follows the same pattern: `eka-naming-and-terminology-specification-v1.1.md`.

| Document | Slug |
|---|---|
| EKA Core Specification v1.1 | `eka-specification-v1.1.md` |
| EKA Exchange Specification v1.1 | `eka-exchange-specification-v1.1.md` |
| EKA Naming and Terminology Specification v1.1 (meta) | `eka-naming-and-terminology-specification-v1.1.md` |
...
- an **Anchor** field naming the anchored document and version (the Exchange Specification pattern: "Engineering Knowledge Architecture (EKA) Canonical Specification v1.1 (Ratified)");
...
|---|---|---|---|---|
| Core | domain | Ratified (v1.1) | self | — |
| Exchange | domain | Ratified (v1.1) | Core §13, §5.4.7, P13 | Core |
...
| Naming and Terminology | meta | Ratified (v1.1) | Core §3 | Core |
...
- **EKA Core Specification** is the official name of the canonical conceptual model document (currently `standard/eka-specification-v1.1.md`).
...
| Rules + tests + fixtures | **Conformance Suite** | the executable form of conformance requirements: the Conformance Rules (R0–R12), their automated tests, and Test Fixtures |
...
- The Conformance Suite is the **executable form of the Conformance Rules** (R0–R12); a future EKA Conformance Specification Family (5.6) will make this binding explicit at the standard level.
...
|---|---|
| Root binary | `eka` |
| Subcommands | **verbs**: `validate`, `init`, `export`, `import`, `view`, `watch`, `version` (implemented); `diagnose`, `sync`, `format`, `graph` (future candidates). `completion` (bash/zsh/fish/powershell) is provided by the Cobra framework — not a core EKA command |
...
| **Conformance Rules** | the canonical rule set **R0–R12** | "validation rules"; "aturan validasi konformitas" (as a name) | R1–R9 = the nine numbered rules (Exchange §14.2 / Aturan 1–9); R0 = structural rule of the Reference Validator (9.3); R10–R12 = domain-aware rules (Core v1.1 §8.1) |
...
| **Engineering Domain** | the home classification of an Artifact: stratum-aligned category of engineering knowledge; derived or declared, never Identity | — | Core v1.1 §8.1; naming discipline: canonical text must write "Engineering Domain", never the bare "Domain" |
| **Domain (bare word)** | forbidden in canonical text: collides with State Domain (Core §7) | "Domain" (as a standalone term) | naming discipline: must write "Engineering Domain" (Core v1.1 §8.1) or "State Domain" (Core §7) |
| **Knowledge Stratum** | the authority level of an Engineering Domain: fixed position in Discovery → Architecture → Planning → Execution → Operations (1 highest → 5); derived from the domain, never declared, never part of the State Vector | — | Core v1.1 §8.1 |
| **Representation Alias** | a methodology term mapped onto a canonical token + Engineering Domain (PRD → req-, Sprint → ctr-); never a frontmatter value, never an Artifact type in its own right | — | Core v1.1 §8.1 |
...
The Conformance Rules set is **R0–R12** (thirteen rules). The phrase "nine rules" refers to **R1–R9** only: the nine rules of EKA Exchange Specification §14.2 remain exactly as defined there. R0 is the structural rule (artifact-rule parsing, frontmatter well-formedness) defined by the Reference Validator; it is not one of the nine rules of EKA Exchange Specification §14.2. **R10–R12 are the domain-aware rules defined by EKA Core Specification v1.1 §8.1** (Engineering Domains and Knowledge Stratification). Documentation must state which set it means; the mapping is:
...
| — (domain-aware bucket) | R10–R12 | EKA Core Specification v1.1 §8.1 |
...
**Revision v1.1:** this revision registers three new terms under the governance above — **Engineering Domain**, **Knowledge Stratum**, **Representation Alias** (Sections 2.3, 9.1) — with no redefinition of any existing term. Terminology additions continue to follow this governance.
...
- Naming changes that affect machine-consumed identifiers (rule IDs, REQ IDs, file slugs, module paths) must follow the stability rules of the artifacts they touch: R0–R12 and REQ-nnn IDs are stable and never reused (CONTRIBUTING.md); slugs change only via the Family registration process.
...
*End of Naming and Terminology Specification — EKA Naming and Terminology v1.1 (Ratified, meta-specification).*
