# EKA Exchange Specification v1.1

| Field | Value |
|---|---|
| **Version** | 1.1 |
| **Revision** | v1.1 — additive: Engineering Domain in unit classification; Conformance Rules set extended to R0–R12 (R10–R12 defined by Core v1.1 §8.1). v1.0 contract unchanged. |
| **Status** | Ratified — milestone 16.1.2 of EKA v1.1 (additive revision; see `reference/ratification-notes-exchange-v1.0.md`) |
| **Anchor** | EKA Core Specification v1.1 (Ratified) |
| **Zone** | standard/ |
| **Scope** | Conceptual exchange contract — not serialization, not implementation |

**Reading this document:** capitalized terms refer to canonical definitions (EKA Core Specification v1.1 Section 3 + glossary) or to Exchange-Spec subordinate terms (Section 4.1). "must" = binding requirement. "should" = recommendation within the bounds of the contract. "may" = option within the bounds of the contract.

## 1. Purpose

1.1 This document is the **Exchange Contract v1** mandated by EKA v1.1 Section 16.1.2: the definition of round-trip, idempotency, referential integrity, schema versioning, and the conformance suite for the Exchange Layer.
...
- Revision v1.1 is additive: it aligns the exchange contract with Core v1.1 §8.1 (Engineering Domains and Knowledge Stratification) — the Engineering Domain member in unit classification and the domain-aware Conformance Rules R10–R12. No v1.0 contract element changes.
...
| Classification | exactly one primary Knowledge Dimension; at most one secondary; Engineering Domain (derived from Type home domain per Core v1.1 §8.1; optional explicit declaration must match) | canonical Section 8; P15 |
...
**Determinism.** The deterministic ordering contract (10.5) applies to every multi-element collection of the object model — Manifest, units, Relationship sets, declaration records, Change Log entries. Two exports from identical repository state **must** yield object models identical up to the permissible differences of 15.4, and therefore identical projections within the same bound. Engineering Domain derivation is deterministic (Core v1.1 §8.1): identical repository state yields identical domain members, so the Classification element stays deterministic.
...
1. **supported specification families** — Families of the EKA standard whose registered contracts the exporter's knowledge conforms to (EKA Naming and Terminology Specification v1.1 §5.6 Family Registry);
...
4. All unit validation uses the taxonomy and state variants of the declared specification version (e.g., Content State variants: standard/ADR/decision, canonical 7.2); Engineering Domain derivation follows the taxonomy of the declared specification version (Core v1.1 §8.1).

### 9.3 Backward compatibility strategy

- Exchange format v1 **must** accept packages written against v1.0 of this specification.
- Contract additions **must** be optional and introduced via declared extensions (Section 16) — never silently change core semantics. The v1.1 additions (Engineering Domain member, domain-aware rules R10–R12) are additive: an absent Engineering Domain is derived from the Type token (14.2 R6), so v1.0 packages remain conformant.
...
- Classification: primary Knowledge Dimension (+ optional secondary) + Engineering Domain (derived from the Type's home domain per Core v1.1 §8.1; explicit declaration optional and must match the token's home domain) (canonical Section 8; P15);
...
| 3 | State validation | values ∈ domain value sets (canonical 7.2, incl. variants); owned-set compliance per type (rule 4); classification: primary Knowledge Dimension declared and valid (rule 6); declared Engineering Domain canonical and matching the Type's home domain (rule 6); forward-only + Change Log consistency (rule 7) | invalid values; foreign domain fields; invalid classification; invalid or mismatched Engineering Domain; log inconsistency | — |
| 4 | Structural validation | Content Well-formed per type (rule 9) | malformed content | — |
| 5 | Referential validation | resolution per 7.4; External References declared; draft tolerance (rule 5); cross-stratum supersession/amendment prohibition (R12, Core v1.1 §8.1 — enforced by the Reference Validator) | unresolved non-draft references; undeclared externals; upward supersession/amendment | unresolved references on Draft artifacts |
...
- Duplicate detection: re-importing an identical package **must** be a no-op — no new units, no state changes, no fabricated Change Log entries, no conflict errors. A unit is a duplicate when its Identity and full payload (Content, State Vector, Change Log, Relationships, Classification) are identical to what the target already holds; identical units are routed to phase 7 (no-op), never to conflict (phase 6). Engineering Domain is derived deterministically (Core v1.1 §8.1), so equivalent repositories produce identical Classification payloads.
...
| R6 | **Classification**: primary Knowledge Dimension declared and valid; secondary optional; the Engineering Domain, when declared, is one of the five canonical domains and equals the Type's home domain (Core v1.1 §8.1); classification changes never touch Identity (P15) | blocking |
...
Notes: the Conformance Rules set is **R0–R12** (Naming and Terminology Specification v1.1 §9.3). The nine rules R1–R9 of this section remain exactly as defined here. R0 (structural rule: artifact-rule parsing, frontmatter well-formedness) is defined by the Reference Validator and is not one of the nine rules. **R10–R12 are the domain-aware rules defined by EKA Core Specification v1.1 §8.1 (Engineering Domains and Knowledge Stratification)** and are enforced by the Reference Validator: R10 — stratification traceability (warning); R11 — domain coherence: a declared Engineering Domain must be canonical and equal the Type's home domain (blocking); R12 — cross-stratum supersession prohibition: `supersedes`/`amends` may never target a strictly higher stratum (blocking). They complement — never replace — the nine rules of this section. In the reference implementation, the superseded-ADR successor check is enforced within Conformance Rule 9 of `validation.md`; enforcement position is a serialization detail — verdict semantics are defined by R5/R9 here and unchanged.
...
| Classification equality | primary and secondary Knowledge Dimensions identical; Engineering Domain identical (derivation is deterministic, so equivalent repositories agree) |
...
- An exchange format v1 importer **must** accept v1.0 packages (9.3); v1.1 packages are exchange format v1 packages with the additive Engineering Domain member.
...
18.2 Conformance suite: a mechanical validator derived from Section 14.2, with verdict semantics fixed (14.3); validators **must** agree (P16). Suite extensions follow governance (canonical 14.2). The complete rule set is R0–R12 (14.2 note): R1–R9 per this section, R0 per the Reference Validator, R10–R12 per Core v1.1 §8.1.

18.3 Extensions: new Artifact types, new Relationship types, new Knowledge Dimensions, and registered protocol variants go through governance (canonical 14.2) and **must** be declared in the package-level Extension Declarations (10.4) when used; extensions **must not** weaken invariants (canonical 14.2.1) and **must** remain exchangeable (canonical 14.2.4). New Artifact types and new Knowledge Dimensions **must** declare their home Engineering Domain (canonical 14.2.7, Core v1.1 §8.1).
...
*End of Exchange Specification — EKA Exchange Specification v1.1 (Ratified, milestone 16.1.2).*
