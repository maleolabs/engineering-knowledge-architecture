# EKA Reference Serialization Format (RSF) v1.1

| Field | Value |
|---|---|
| **Version** | 1.1 |
| **Title** | EKA Reference Serialization Format (RSF) v1.1 |
| **Status** | Reference (not normative) |
| **Anchor** | EKA Exchange Specification v1.1 (Ratified) |
| **Zone** | reference/ |
...
**Reading this document:** capitalized terms refer to canonical definitions (EKA Core Specification v1.1 Section 3 + glossary) or to Exchange-Spec subordinate terms (EKA Exchange Specification v1.1 §4.1). Terms marked "(RSF)" are defined in Section 3; they are subordinate to the Exchange Specification and never canonical. "must" = binding requirement within the RSF. "should" = recommendation within the bounds of the RSF. "may" = option within the bounds of the RSF.

## 1. Purpose

1.1 The **EKA Reference Serialization Format (RSF) v1.1** is one canonical implementation of the Exchange Contract: a concrete serialization projection of the Canonical Exchange Package Object Model (EKA Exchange Specification v1.1 §4.4). It makes the object model — Contract Header, Manifest, Exchange Units, Declarations, integrity data — serializable in a deterministic, lossless, machine-parseable form.

1.2 The RSF is **not normative**. It lives in the Reference Zone as meta-documentation of the Reference Implementation (EKA Naming and Terminology Specification v1.1 §6.1). It is not an extension of the Exchange Specification, does not amend it, and is not a new Specification Family (Naming §5.6). The Exchange Specification remains the sole normative contract; the RSF is one of its possible projections (Exchange §2.3, §4.4).
...
- redefinition of the Conformance Rules R0–R12 (Exchange §14.2) or the addition of new EKA concepts;
...
| **Representation Alias (RSF)** | optional unit metadata labeling a methodology representation (e.g., "prd", "sprint") mapped onto the canonical Type token (Core v1.1 §8.1); metadata only — never Identity, never an ordering key (Section 5.4) |
...
| **Serialization Version (RSF)** | the version of the RSF itself (v1.1); distinct from the exchange format version and the specification version (Section 11.2) |
...
One Unit Entry (RSF) per Exchange Unit (Exchange §5.2). The composition is exactly the object-model unit composition (Exchange §4.4), plus the RSF-subordinate Attachment Reference Set and the optional Representation metadata:
...
| Representation metadata | zero or one | optional alias label (Representation Alias (RSF), Section 5.4); unit metadata, never Identity, never an ordering key |
...
| Classification | exactly one primary Knowledge Dimension; at most one secondary; Engineering Domain (derived from Type home domain per Core v1.1 §8.1; optional explicit declaration must match) | canonical Section 8; P15; classification never touches Identity (R6) |
...
5.3.2 Classification carries the primary Knowledge Dimension plus at most one secondary, and the Engineering Domain of the unit's artifact type, derived from the Type token's home domain (Core v1.1 §8.1); when the Engineering Domain is declared explicitly it must equal the Type's home domain (Exchange §14.2 R6). Classification changes never affect Identity (P15; Exchange §14.2 R6).
...
### 5.4 Revision, unit metadata, and Representation Alias
...
5.4.3 The **Representation Alias (RSF)** (Section 3) travels as unit metadata: like Revision, it is serialized with the Unit Entry and round-trips with the unit. It is never part of Identity (canonical 6.1), never an ordering key, and never a validity condition — present or absent, the canonical Type token alone governs Identity and State (Core v1.1 §8.1: Representation Aliases are methodology labels, never Artifact types in their own right).
...
Attachments (RSF) are supporting resources: diagrams, images, binaries, PDFs, other assets. They are never part of Content semantics — Content remains complete and Well-formed without them; attachments are supporting resources referenced from units, never inlined into Content. They carry no Identity, State Vector, Change Log, Classification, Phase, or unit metadata (Revision, Representation Alias): they are not Exchange Units and realize no object model element (Appendix A).
...
- Attachment handling never changes exchange-conformance verdicts (R0–R12 apply to units); import decisions on Attachments are recorded in the Import Manifest (Exchange §11.3).
...
Consequence: identical repository state must produce identical packages (Exchange §10.5). The Engineering Domain member is derived deterministically from the Type token (Core v1.1 §8.1), so it participates in the determinism guarantee like every other Classification field.
...
- a v1.0 package (Serialization Version 1) is **not** a forward-compatibility violation: the Engineering Domain member and the Representation metadata are optional v1.1 additions; the importer derives the Engineering Domain from the Type token at import (Exchange §9.2.4) and treats absent Representation metadata as absent (Section 11.2).
...
| Classification equality | Classification fields: primary + secondary Knowledge Dimensions + Engineering Domain (Core v1.1 §8.1) |
| Line integrity | instance grouping: Unit Entries grouped by (Namespace, Type, ID) across InstanceVersions |
| Representation metadata (unit metadata; §5.4) | Unit Entry: Representation metadata — alias label (Representation Alias (RSF)); round-trips with the unit |
...
**Serialization Version 1.1** is additive over 1: it adds the Engineering Domain member to the unit Classification (Exchange §4.4, §10.3) and the optional Representation metadata (Section 5.4). v1.0 packages remain importable: an RSF v1.1 importer accepts Serialization Versions **1 and 1.1**; a v1.0 package carries no Engineering Domain member, so the importer derives the domain from the Type token at import (Exchange §9.2.4), and carries no Representation metadata.

### 11.3 Negotiation

- the Package Header declares the exchange format version, the specification version, and the Serialization Version;
- an importer must reject an unsupported Serialization Version before validation phases (analogous to Exchange §9.2.2, applied at the parse stage): an RSF v1.1 importer accepts Serialization Versions 1 and 1.1 (11.2); versions above 1.1 or unknown versions are rejected;
...
1. validate the source repository (Reference Validator, R0–R12) — Exchange §12.5;
...
- Engineering Domain derived from the Type token (Core v1.1 §8.1) — never read from environment, never guessed.
...
| Exchange Unit: Revision metadata (unit metadata) | Unit Entry: unit metadata: Revision metadata + Representation Alias (RSF) — optional alias label (§5.4) |
...
| Exchange Unit: Classification | Unit Entry: Classification fields (primary + secondary Knowledge Dimensions; Engineering Domain member) |
...
| — (no object model element) | Package Identity Label, Attachments Collection, Attachment Reference Set, Representation metadata (alias label; RSF support element — unit metadata, §5.4), Serialization Version (RSF support elements; Section 3) |

## Appendix B — RSF v1.1 Conventions Summary

| Convention | v1.1 value |
|---|---|
| Serialization Version | 1.1 (additive over 1; importer accepts 1 and 1.1; v1.0 packages derive the Engineering Domain at import) |
...
| Engineering Domain | derived from the Type token (Core v1.1 §8.1); written on export; explicit declarations must match (Exchange §14.2 R6) |
| Representation Alias | optional unit metadata label (e.g., "prd", "sprint"); never Identity, never an ordering key (§5.4) |
...
*End of RSF — EKA Reference Serialization Format v1.1 (Reference, not normative).*
