...
| `exchange/` | root (tooling) | konvensi (engine) | Exchange Spec 4.4, 10, 11, 12, 15; RSF | Engine import/export (application layer, package publik): discovery, loading (via `conformance.Scan`), scope resolution (repo/line/instance/collection), external reference declaration, proyeksi RSF deterministik, deserialization + verifikasi integritas, resolusi identity/relationship, conflict analyzer, integration engine (staged commit + rollback), guard charset identity (RSF §5.2.3). |
