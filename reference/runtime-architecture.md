...
> Authoritative decisions: [ADR-009](decisions/adr-009-knowledge-runtime-architecture.md) (workspace + embedded canonical store), [ADR-010](decisions/adr-010-synchronization-model.md) (synchronization protocol), [ADR-011](decisions/adr-011-immutable-engineering-knowledge-model.md) (the Immutable Engineering Knowledge Model — schema v2), [ADR-012](decisions/adr-012-canonical-knowledge-object-runtime.md) (the Canonical Knowledge Object Runtime — the consumption side, §2.1), [ADR-013](decisions/adr-013-store-backed-projections.md) (store-backed projections — projections read the canonical store, §2.1/§8/§11), [ADR-014](decisions/adr-014-runtime-interface-architecture.md) (the Runtime Kernel — the runtime/ interface architecture, §2.2/§11), and [ADR-015](decisions/adr-015-machine-retrieval-interface.md) (the machine interface — `eka get`, §2.3/§11). This document summarizes and explains; the ADRs are normative for this implementation.
...
└─────────┬──────────┘
          ▼
┌────────────────────┐
│ Human / Machine /  │   terminal projections (eka view / eka watch) and canonical
│        AI          │   CKO JSON (eka get) today; MCP JSON · Atrium (future)
└────────────────────┘
```
...
| **CKO consumers** | `view/` + `cmd` | projections over `exchange.Unit` read **from the workspace canonical store** — `eka view` / `eka watch` resolve the project via the runtime services (`Knowledge.UnitsByProject` — the Runtime Kernel, ADR-014; `store.UnitsByProject` behind it: refs → immutable payloads → `exchange.DecodeUnit`, canonical-form order) and never read Markdown; the compile path is gone from `cmd/view.go` / `cmd/watch.go`; **zero Markdown parsing in the consumption path** (the `view` package imports no `conformance.Scan`/`Artifact`) |
| **Machine serializer** | `machine/` + `cmd/get.go` | CKO → **canonical JSON** (schema `eka-cko-v1`): never renders, never parses Markdown, never touches storage — the machine path is a pure function of the CKO, read via the Runtime API (`Knowledge.Search`, `Resolver.Resolve`, `Workspace.FindRepo`; ADR-015) |
...
The ontology helpers remain shared and representation-independent: `conformance.ParseReference` (reference resolution), `DomainValues`/`OwnedDomains` (state-domain taxonomy), `DomainForToken`/`Stratum` (domain/stratum derivation). Presentation is a separate concern: terminal projections today (`eka view` / `eka watch`) and canonical CKO JSON via the machine interface (`eka get`, ADR-015); future presentations (MCP JSON, Atrium components) consume the same CKO contract. The docs-mode pull (sync) goes through the compiler — the authoring gateway; projections (`eka view` / `eka watch`) read the canonical store through the runtime services (`Knowledge.UnitsByProject` — the Runtime Kernel, ADR-014, §2.2), so the runtime consumption path never touches Markdown and never re-runs the authoring gate per invocation (ADR-013).

### 2.2 The Runtime Kernel: services, not storage

Since ADR-014, **every** consumer communicates with the runtime through the **Runtime Kernel** (package `runtime/`) — internal, in-process service contracts — never through storage internals. Direct database access outside the Runtime is no longer valid architecture, enforced structurally by the import graph. The Kernel exposes two APIs ([`runtime-api.md`](runtime-api.md) documents both; ADR-014 is authoritative):

- **The Runtime API** — seven concrete-type services: **Workspace** (registry, last sync, aggregated status), **Knowledge** (`Object` / `UnitsByProject` / `Units` / `Search` / `Counts` — Engineering-Knowledge reads returning `exchange.Unit`, the CKO), **Resolver** (`Resolve` / `ResolveLine`), **Relations** (`From` / `To` / `Upstream` / `Downstream` — consumers never hand-walk relationships), **Timeline** (`Line`), **Snapshot** (`Read`), **Integrity** (`Verify` / `SchemaVersion`). Knowledge-shaped, never CRUD/storage-shaped, never Markdown-aware; concrete types by design — no Go interface types until a consumer needs polymorphism, no HTTP/RPC (one process, one binary).
- **The Authoring API** — the stateless `AuthoringService` (`runtime.Authoring`): `Validate` (R0–R12 gate), `Compile` (authoring → CKO via the Knowledge Compiler), `Sync` (compile + seed the canonical store + snapshot write — the explicit synchronization of ADR-010). Markdown is one authoring adapter behind it; future adapters enter through the same API.

The **CLI is a runtime client**: every runtime-touching command delegates to the services (`eka sync` → `Authoring.Sync`; `eka view` / `eka watch` → `Knowledge.UnitsByProject` + the projection renderer; `eka status` → `Workspace.Status`; `eka project` → the Workspace service; `eka integrity check` → the Integrity service; `eka validate` → `Authoring.Validate`). **Output and exit codes are unchanged** — all pinned CLI tests pass. `eka init` stays the authoring adapter (bootstrap/); `eka export` / `eka import` stay exchange transport operations (no runtime store access).

**Storage isolation:** SQLite (`store/`) is a **private persistence implementation** of the Kernel. The `store/`, `workspace/`, `sync/` and `compile/` packages are internal to the Kernel — production `cmd/` must not import them; the persistence handle (`workspace.Store()`, replacing the exported `Workspace.DB` field) is Kernel-internal. Tests may use `store`/`workspace` directly for seeding and corruption fixtures (test-only, documented). The isolation is structural — checkable by `go list` / an import linter — not a convention. One-way dependencies, no cycles: `runtime` → {`store`, `workspace`, `sync`, `compile`, `conformance` (helpers), `exchange`}; `cmd` → {`runtime`, `exchange`, `view`, `conformance` (types), `bootstrap`, `ui`}.

### 2.3 The machine interface: `eka get`

The consumption pipeline ends in two deliberately separated paths ([ADR-015](decisions/adr-015-machine-retrieval-interface.md)) — human and machine are different consumers of the same Runtime API:

```
Runtime DB → Runtime API → projection path: view/ models → renderers → terminal   (eka view / eka watch)
Runtime DB → Runtime API → machine path:   machine/ serialization → JSON → stdout (eka get)
```

- **The machine path** (`cmd/get.go` → `machine/` → Runtime API) retrieves Engineering Knowledge as **canonical JSON generated directly from Canonical Knowledge Objects** — `Knowledge.Search` (domain queries), `Resolver.Resolve` (identity lookups), `Workspace.FindRepo` (the repository-state gate). It **never renders for readability, never parses Markdown, never queries SQLite, never reuses projection renderers**; it is read-only end to end (`runtime.Open`, never `runtime.Ensure` — a missing workspace is a refusal, exit `1`, not an initialization).
- **The separation is formal** (ADR-015 §Decision 5): the machine path imports `{runtime, exchange}` and never imports `view/`; the projection path never imports `machine/`. The two share only the Runtime API and the CKO model — a rendering change in `view/` can never touch the machine contract, and both paths are pure functions of the same objects, so they cannot drift.
- **The JSON contract** (schema `eka-cko-v1`, stable across minor releases): one CKO = one Document (identity + canonical form, derived domain/stratum, state vector, classification, relationships, change-log, content verbatim, object hash); a domain query returns a Collection `{schema, collection: "domain", domain, count, units}` sorted by canonical form. Deterministic: fixed field order, no timestamps — identical synced store state → byte-identical JSON. stdout carries only the JSON (+ one trailing newline); errors → stderr; exit 0 / 1 (workspace or repository-state refusal, mirroring `eka view`) / 2 (usage, unknown identity, internal).
- **The query surface is knowledge-shaped**: a target containing `:` is an identity lookup (RSF canonical form or qualified line form; the namespace is required — unqualified forms are refused); a target without `:` names one of the five Engineering Domains. No flags in v0.2; future targets (traversal, timeline, filters) grow over the same Runtime API and keep the JSON `eka-cko-v1`-compatible (ADR-015 §Decision 6).

The full reference (query model, field table, exit codes, examples) is [`cli.md`](cli.md); the serializer is the `machine/` package.
...
- **Registered project without synced knowledge** — `Knowledge.UnitsByProject` returns nothing: the projection renders empty with the informational note `no synced knowledge for project <id>; run 'eka sync' after editing docs`, exit `0` — consistent with the existing empty-projection behavior.

`eka get` (ADR-015, §2.3) shares the same workspace/repository-state gate: a missing workspace (detached `runtime.Open` — `eka get` never creates one) or an unregistered repository is a refusal with a deterministic message + hint, exit `1` — the same refusal class as `eka view` (ADR-013 §Decision 3).

Authoring validation (R0–R12) runs inside sync (the compile gate); projections never re-validate authoring per invocation — the correctness of the projected units is the store's integrity contract, checked by `eka integrity check` ([§6](#6-integrity)). Authoring UX: **write Markdown → `eka sync` → `eka view`**. For machine consumers the same UX applies: **write Markdown → `eka sync` → `eka get`**.
...
| `eka view` | **store-backed** — reads Canonical Knowledge Objects **from the workspace canonical store via the runtime services** (`Knowledge.UnitsByProject` → `exchange.DecodeUnit`), projecting the **complete knowledge of the project** (the union of every registered repository's units); requires a **registered and synced** repository (run `eka sync` first): unregistered → **refused**, exit `1` (deterministic message + hint); registered-but-unsynced → empty projection + informational note, exit `0`; `compile` is no longer imported (ADR-013) | **revised** — sync-first precondition (was: compile-on-demand); ADR-013 |
| `eka watch` | **store-backed** — polls the canonical store (`Knowledge.UnitsByProject` per tick, `--interval` unchanged); TTY contract and byte-comparison redraw unchanged; the **unregistered-repository refusal frame** replaces the compile-failure frame; keeps polling until registered + synced (ADR-013 §Decision 5) | **revised** — sync-first precondition (was: compile-on-demand); ADR-013 |
| `eka get <target>` | **new** — the machine interface: canonical CKO JSON (schema `eka-cko-v1`) via the Runtime API (`Knowledge.Search`, `Resolver.Resolve`, `Workspace.FindRepo`) + the `machine/` serializer; never renders, never parses Markdown, never touches storage; identity lookup (canonical/qualified line form, namespace required) or domain query (the five Engineering Domains → Collection, sorted by canonical form); stdout carries only the JSON, errors → stderr; exit `0` / `1` (workspace or repository-state refusal, mirroring `eka view`) / `2` (usage, unknown identity, internal); read-only, never creates a workspace | new (ADR-015) |
...
**Delegation (ADR-014):** every runtime-touching command is a **client of the Runtime Kernel** ([§2.2](#22-the-runtime-kernel-services-not-storage), [`runtime-api.md`](runtime-api.md)) — `eka sync` → `Authoring.Sync`; `eka view` / `eka watch` → `Knowledge.UnitsByProject` + the projection renderer; `eka get` → `Resolver.Resolve` / `Knowledge.Search` + the machine serializer (`machine/`, ADR-015); `eka status` → `Workspace.Status`; `eka project register`/`list` → the Workspace service's registry surface; `eka integrity check` → the Integrity service; `eka validate` → `Authoring.Validate`. `eka init` stays the authoring adapter (bootstrap/); `eka export` / `eka import` stay exchange transport operations (no runtime store access). **Behavior, output, and exit codes are unchanged** — the commands merely stop touching storage internals; all pinned CLI tests pass.

## 12. Future roadmap (NOT implemented in v0.2)

The schema and protocol leave structural room for all of these; none of them exist yet — **except store-backed projections and the machine retrieval baseline, implemented in v0.2** (ADR-013, ADR-015; see §2.1, §2.3 and §8). One paragraph each on how the runtime supports the rest (ADR-011 §Decision 8 reserves the same room):
...
- **Runtime Kernel / Interfaces — DONE (v0.2)** — the `runtime/` package exposes the internal **Runtime API** (Workspace, Knowledge, Resolver, Relations, Timeline, Snapshot, Integrity) and the **Authoring API** (`runtime.Authoring`: Validate/Compile/Sync); the CLI is a client of the services; SQLite is a private persistence detail behind the Kernel (ADR-014; see [`runtime-api.md`](runtime-api.md) and §2.2). Every remaining roadmap item below builds on these contracts.
...
- **Knowledge Timeline** — the change-log array inside each immutable `unit.json` payload carries the per-form transitions (date, domain, from, to, by) in occurrence order; a timeline is a projection over payloads. The Kernel exposes it as `Timeline.Line` (ADR-014); no timeline command exists.
- **Context Engine** — metadata queries over the `object_refs` index columns (`dimension`, `domain`, `phase`, provenance, identity tuple) are point lookups on existing indexes. Its query surface on the Kernel is `Knowledge.Search` + `Relations` traversal (ADR-014 §Future Extensibility); v0.2 exposes only `eka status` counts.
- **Machine-readable API — DONE (v0.2, baseline)** — `eka get` (ADR-015): the machine interface retrieves CKO as **canonical JSON** (schema `eka-cko-v1`) directly from the Runtime API — `Knowledge.Search`, `Resolver.Resolve`, `Workspace.FindRepo` — serialized by the `machine/` package, emitted on stdout, never rendered, never parsed from Markdown, never touching storage. This is the read-only machine baseline ADR-011 §Decision 8 / ADR-014 §Future Extensibility reserved. What remains future is the **wire layer** (HTTP/REST/gRPC or an MCP server wrapping the same services and serialization) and extended query targets (relationship traversal, timeline/history, semantic search, metadata filtering, context generation — additive over the same Runtime API, JSON stays `eka-cko-v1`-compatible; ADR-015 §Decision 6).
...
- **MCP Integration** — an MCP server is a **thin adapter over the runtime services** (ADR-014 §Future Extensibility): it consumes CKO through the Runtime API without learning storage — the storage packages are kernel-internal. Not implemented.
...
- **Atrium (unified project runtime)** — the cross-repository union with `source_repo` provenance is the exact data shape a project-wide consumer needs: one project, many repositories, complete Engineering Knowledge in one indexed store. Its read path on the Kernel is the projection source (`Knowledge.UnitsByProject`) + the resolver (`Resolver.Resolve`/`ResolveLine`) (ADR-014 §Future Extensibility). Atrium is the consumer this architecture was inverted for; it is not built.
...
- [ADR-014 — Runtime Interface Architecture](decisions/adr-014-runtime-interface-architecture.md) — the Runtime Kernel (`runtime/`): the internal Runtime API + Authoring API; the CLI as a client; SQLite as private persistence; structural isolation
- [ADR-015 — Machine Retrieval Interface](decisions/adr-015-machine-retrieval-interface.md) — the machine interface (`eka get`): canonical CKO JSON (schema `eka-cko-v1`), query model (identity lookup / domain query), output contract, formal separation from projections (ADR-014 deliberately not amended)
- [`runtime-api.md`](runtime-api.md) — the Runtime Kernel interfaces: service × operation contracts, determinism and scan-based trade-offs, storage isolation rules, package organization, future integrations
