...
> Authoritative decisions: [ADR-009](decisions/adr-009-knowledge-runtime-architecture.md) (workspace + embedded canonical store), [ADR-010](decisions/adr-010-synchronization-model.md) (synchronization protocol), [ADR-011](decisions/adr-011-immutable-engineering-knowledge-model.md) (the Immutable Engineering Knowledge Model — schema v2), [ADR-012](decisions/adr-012-canonical-knowledge-object-runtime.md) (the Canonical Knowledge Object Runtime — the consumption side, §2.1), [ADR-013](decisions/adr-013-store-backed-projections.md) (store-backed projections — projections read the canonical store, §2.1/§8/§11), and [ADR-014](decisions/adr-014-runtime-interface-architecture.md) (the Runtime Kernel — the runtime/ interface architecture, §2.2/§11). This document summarizes and explains; the ADRs are normative for this implementation.
...
- **Machine-readable API** — a read-only API over the runtime services: CKO serialization over the same contracts, a thin adapter, exactly as ADR-011 §Decision 8 reserved (ADR-014 §Future Extensibility). The store is a single SQLite file with a stable schema, made trivially safe for concurrent readers by immutability; the schema is deliberately public (ADR-009) so the API can be added without redesign.
