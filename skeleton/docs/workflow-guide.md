...
| `eka sync` | Synchronize | Seeds the EKA workspace canonical store from this repository (registers it) and writes the Knowledge Snapshot — run once before projecting |
| `eka view` | Project | Renders read-only projections per domain, plus the ticket and board views (Section 10); reads the workspace store, so `eka sync` must run first |
...
| `eka view board` | **Every work item in the project** — across all containers (active and completed) and outside any container — on the same five-column Kanban board; each item is tagged with its container, and items no ticket references show `(unassigned)`. For "what is the total work?" instead of "what is being worked on right now?" |
